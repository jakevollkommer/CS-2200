The Conte-200 Assembler and Simulator
===============

To aid in testing your processor, we have provided an *assembler* and
a *simulator* for the Conte-200 architecture. The assembler supports
...
the assembler. The Conte-200 assembler definition is included.

Included Files
-----------

* `assembler.py`: the main assembler program
* `conte200.py`: the Conte-200 assembler definition
* `conte200-sim.py`: the Conte-200 simulator program
...
Example usage to assemble `test.s`:
    ...
    python assembler.py -i conte200 --sym test.s
...
To output assembled code in hexadecimal (for use with *Brandonsim*):
    ...
    python assembler.py -i conte200 --hex test.s
...
Example usage to simulate `test.bin` (generated by the assembler):
    ...
    python conte200-sim.py test.bin
...
In addition to the syntax described in the Conte-200 ISA reference,
